...
* In "Database" use these values
   ...
   * Leave "Use passwordless datasource connection" unchecked.
...
* Enable public network to access the admin server. Find and click on wls-nsg. In the Overview page, find `WebLogicAdminPortsAllowed` under the Inbound Security Rules, click on it. In the pop window, select `Any` for Source and click Save, wait for the update to be completed.
* Find `WebLogicManagedChannelPortsDenied` under the Inbound Security Rules, click on it. In the pop window, select `Allow` for Action and click `Save`, wait for the update to be completed. This will open portal 8501 which we will use later.
